...
function timeout()
{
    // We need to set the timeout *first*, otherwise if we try to clear it later, we will get exceptions
...
function AnimationManager(objectManager)
{
    ...
    this.startNextBlock = function()
    {
        ...
        //  If there are not any animations, and we are currently paused,
    ...
    this.stepBack = function()
    {
        if (this.awaitingStep && this.undoStack != null && this.undoStack.length != 0)
        {
            ...
            this.awaitingStep = false;
            this.undoLastBlock();
            // Re-set the timer.  The timer may or may not be running at this point,
            // so to be safe we'll stop it and start it again.
            clearTimeout(timer);
            timer = setTimeout('timeout()', 30);
