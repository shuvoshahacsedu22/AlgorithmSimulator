...
function initCanvas()
{
	...
	//set cookie values for animation speed slider
	$(element).slider({
					  animate: true,
					  value: speed,
...
function AnimationManager(objectManager)
{
	...
	// Array holding the code for the animation.  This is an array of strings,
	// each of which is an animation command currentAnimation is an index of this array
	this.AnimationSteps = [];
	...
	this.startNextBlock = function()
	{
		...
		{			
			var nextCommand = this.AnimationSteps[this.currentAnimation].split("<;>");
			 ...
			 if (nextCommand[0].toUpperCase() == "CREATERECTANGLE")
			{
				if (nextCommand.length == 9)
				{
					this.animatedObjects.addRectangleObject(parseInt(nextCommand[1]), // ID
															...
															"#97ffd9", // background color
					                                        "#000000"); // foreground color
				}
				else if (nextCommand[7]=="Queue"){
                    this.animatedObjects.addRectangleObject(parseInt(nextCommand[1]), // ID
                        nextCommand[2], // Label
                        parseInt(nextCommand[3]), // w
                        parseInt(nextCommand[4]), // h
			...
			else if (nextCommand[0].toUpperCase() == "CREATEHIGHLIGHTCIRCLE")
			{
				...
				{
					this.animatedObjects.addHighlightCircleObject(parseInt(nextCommand[1]), this.parseColor(nextCommand[2]), 15);
				}
	...
	//			throw "Unknown command: " + nextCommand[0];					
		...
		this.currFrame = 0;
		//if paused currFrame points to the next block
