...
QueueLL.prototype.addControls =  function()
{
    ...
    this.enqueueRandomButton = addControlToAlgorithmBar("Button", "Enqueue Random Value");
    this.enqueueRandomButton.onclick = this.enqueueRandomCallback.bind(this);
    this.controls.push(this.enqueueRandomButton);
...
QueueLL.prototype.clearData = function(ignored)
{
    this.commands = new Array();
    this.cmd("StopAnimation");
	while (this.top>0) {
        var labPopID = this.nextIndex++;
        var labPopValID = this.nextIndex++;
    ...
    this.cmd("RestoreAnimation");

    return this.commands;
}
QueueLL.prototype.enqueueRandomCallback = function(event) {

    if (this.top < SIZE) {
        var pushVal = Math.floor(Math.random() * 20);
        this.implementAction(this.enqueue.bind(this), pushVal);
    }
...
QueueLL.prototype.clearAnimationFlag = function () {
    animationFlag=false;
}
QueueLL.prototype.setAnimationFlag = function () {
    animationFlag=true;
}
...
function init()
{
    ...
    animManag.addListener("AnimationEnded", this, this.clearAnimationFlag());
    animManag.addListener("AnimationStarted", this, this.setAnimationFlag());
}
