...
{
    if (this.maxDegree != newMaxDegree)
    {
...
BTree.prototype.printTree = function(event)
{
    this.commands = new Array();
...
BTree.prototype.changeDegree = function(degree)
{
    ...
    this.maxDegree = newDegree;
    this.maxKeys = newDegree - 1;
    this.minKeys = Math.floor((newDegree + 1) / 2) - 1;
    this.splitIndex = Math.floor((newDegree - 1) / 2);
...
BTree.prototype.insertElement = function(insertedValue)
{
    ...
    else
    {
        if (this.preemptiveSplit)
        {
            if (this.treeRoot.numKeys == this.maxKeys)
            {
...
BTree.prototype.insertNotFull = function(tree, insertValue)
{
    ...
    else
    {
        ...
        this.cmd("SetHighlight", tree.graphicID, 0);
        if (tree.children[findIndex].numKeys == this.maxKeys)
        {
...
BTree.prototype.split = function(tree)
{
    ...
    rightNode.numKeys = tree.numKeys - this.splitIndex - 1;
    var risingNode = tree.keys[this.splitIndex];
    ...
    if (tree.parent != null)
    {
        ...
        this.moveLabel1ID = this.nextIndex++;
        this.cmd("CreateLabel", this.moveLabel1ID, risingNode, this.getLabelX(tree, this.splitIndex),  tree.y)
    ...
    this.cmd("CreateBTreeNode",
        rightNode.graphicID,
        WIDTH_PER_ELEM, NODE_HEIGHT,
        tree.numKeys - this.splitIndex - 1,
        tree.x,
        tree.y,
        BACKGROUND_COLOR,
        FOREGROUND_COLOR);
    ...
    var i;
    for (i = this.splitIndex + 1; i < tree.numKeys + 1; i++)
    {
        rightNode.children[i - this.splitIndex - 1] = tree.children[i];
        if (tree.children[i] != null)
        {
            ...
            this.cmd("Connect", rightNode.graphicID,
                rightNode.children[i - this.splitIndex - 1].graphicID,
                FOREGROUND_COLOR,
                0, // Curve
                0, // Directed
                "", // Label
    ...
    for (i = this.splitIndex+1; i < tree.numKeys; i++)
    {
        rightNode.keys[i - this.splitIndex - 1] = tree.keys[i];
        this.cmd("SetText", rightNode.graphicID, rightNode.keys[i - this.splitIndex - 1], i - this.splitIndex - 1);
    }
    var leftNode = tree;
    leftNode.numKeys = this.splitIndex;
    
    for (i = this.splitIndex; i < tree.numKeys; i++)
    {
        this.cmd("SetText", tree.graphicID, "", i);
    }
    // TO MAKE UNDO WORK -- CAN REMOVE LATER ^^
    this.cmd("SetNumElements", tree.graphicID, this.splitIndex);
    ...
    if (tree != null)
    {
        ...
        if (i == tree.numKeys)
        {
            if (!tree.isLeaf)
            {
                ...
                if (tree.children[tree.numKeys].numKeys == this.minKeys)
                {
                    var nextNode;
                    if (tree.children[tree.numKeys - 1].numKeys > this.minKeys)
                    {
        ...
        else if (tree.keys[i] > val)
        {
            if (!tree.isLeaf)
            {
                ...
                if (tree.children[i].numKeys > this.minKeys)
                {
                    this.doDeleteNotEmpty(tree.children[i], val);
                }
                else
                {
                    if (tree.children[i+1].numKeys > this.minKeys)
                    {
            ...
            else
            {
                ...
                if (tree.children[i].numKeys == this.minKeys)
                {
                    ...
                    else
                    {
                        ...
                        while (!minNode.isLeaf)
                        {
                            ...
                            if (minNode.children[0].numKeys == this.minKeys)
                            {
                                if (minNode.children[1].numKeys == this.minKeys)
                                {
                ...
                else
                {
                    ...
                    while (!maxNode.isLeaf)
                    {
                        ...
                        if (maxNode.children[maxNode.numKeys].numKeys == this.minKeys)
                        {
                            if (maxNode.children[maxNode.numKeys - 1] > this.minKeys)
                            {
...
BTree.prototype.repairAfterDelete = function(tree)
{
    if (tree.numKeys < this.minKeys)
    {
        ...
        else
        {
            ...
            if (parentIndex > 0 && parentNode.children[parentIndex - 1].numKeys > this.minKeys)
            {
                this.stealFromLeft(tree, parentIndex);

            }
            else if (parentIndex < parentNode.numKeys && parentNode.children[parentIndex + 1].numKeys > this.minKeys)
            {
